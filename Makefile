...
# contents — is passed in via RUNONSPARK_SIGN_KEY so the key material never
# appears as a command-line argument either. See cmd/sign-index/main.go for
# the key file format and how to generate one.
#
# Usage:
#   RUNONSPARK_SIGN_KEY=/path/to/private.key make sign-index INDEX=path/to/index.json
.PHONY: sign-index
sign-index:
	@if [ -z "$$RUNONSPARK_SIGN_KEY" ]; then \
		echo "sign-index: set RUNONSPARK_SIGN_KEY to the path of the ed25519 private key file (never its contents)" >&2; \
		exit 1; \
	fi
	go run ./cmd/sign-index -index "$${INDEX:-index.json}" -key "$$RUNONSPARK_SIGN_KEY"
