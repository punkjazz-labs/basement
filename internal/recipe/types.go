...
type SGLangConfig struct {
	...
	// SGLang pins 64 itself when it serves compressed sparse attention, which
	// addresses its cache in whole pages, so the flag states a number the
	// runtime already holds rather than changing it. A recipe writes it down
	// because the schema's other page rules are checked against it. 0 leaves
	// the flag off and SGLang's own default stands.
	PageSize int `yaml:"page_size" json:"page_size"`
	// MambaTrackInterval is --mamba-track-interval, the interval SGLang
	// tracks Mamba state on. The qualified launcher does not say what the
	// runtime does between two of them, so neither does this comment. What it
	// does state are the two rules that follow from the state being addressed
	// in whole cache pages: the interval is a multiple of the page size, and
	// it is never smaller than the draft token count. A recipe that sets this
	// must pin page_size too, which is the only shape a launcher has run, and
	// the validator then holds the recipe to the multiple. A page of 64 keeps
	// every interval it admits above the 32 draft tokens this schema allows,
	// so the second rule cannot be broken. 0 leaves the flag off and SGLang's
	// own default stands.
	...
	// refusing it. It is one of the settings a model card's own validated
	// TP=2 serving recipe lists, and the qualified launcher passes it on
	// every start, so one over-long prompt comes back as an answer rather
	// than as an error.
	...
	// of those tables in FP8. The qualified launcher offloads them on a GB10
	// and calls that placement validated there: it costs about 26 GB of
	// pinned host memory per rank at TP=2. Keeping them device-resident costs
	// about the same again in device memory per rank, which only fits beside
	// a lower mem_fraction_static.
	...
	// numbers separated by whitespace. A recipe pins it because the runtime's
	// own list is chosen for a larger device: on a GB10 the default list runs
	// the graph pool out of memory, so a qualified launcher names the few
	// batch sizes it really serves and extends that list alongside
