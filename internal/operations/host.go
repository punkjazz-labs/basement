...
func (h *HostExecutor) Execute(ctx context.Context, execution Execution, operation recipe.Operation, r recipe.Recipe, progress Progress) (map[string]any, error) {
	...
	case "write_generated_config":
		...
		config := map[string]any{"recipe_id": r.ID, "recipe_version": r.Version, "image": r.Runtime.Reference(), "model_revisions": modelRevisions, "container_name": containerName(r), "arguments": vllmArgs(r, execution.Placement)}
