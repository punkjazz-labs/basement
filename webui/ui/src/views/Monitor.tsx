...
import { machineSeries, subscribeTelemetry, telemetryRevision, type MachineSeries } from '../telemetry'
...
// opens with the lines already on the charts.

// One Spark, as this page names it.
export interface MonitorMachine {
  // The key its samples are kept under: this Spark, or a peer id.
  key: string
  name: string
}
...
function MachineSection({ machine, held, recipes }: {
  ...
  const dot = quiet ? 'fail' : active ? 'on' : ''
  const line = quiet
